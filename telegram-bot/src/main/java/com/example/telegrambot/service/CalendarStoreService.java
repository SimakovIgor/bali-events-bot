...
public class CalendarStoreService {
    ...
    public LocalDate putOrUpdate(Update update) {
        ...
        LocalDate dateToStore = LocalDate.now();                    // по умолчанию текущая дата

        if (calendarStore.containsKey(chatId)) {
            try {
                dateToStore = LocalDate.parse(text, DATE_TIME_FORMATTER);
            } catch (DateTimeParseException e) {
                // Если текст не удалось распарсить как дату, оставляем текущую дату
            }
        }
    ...
    public LocalDate get(Update update) {
        return calendarStore.get(update.getMessage().getChatId());  // получаем дату запроса пользователя
    }
